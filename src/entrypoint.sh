...
#   2. Apply the EULA gate and the few convenience shortcuts (BUNGEECORD).
#   3. Assemble the JVM command line (memory + a flags preset + user overrides).
#   4. exec mc-server-init as PID 1 (own repo github:Team-MaRo/mc-server-init, a
...
# we simply refuse to launch unless it was accepted.
...
# Full escape hatch: a CMD/args override replaces the launch entirely.
if [ "$#" -gt 0 ]; then
